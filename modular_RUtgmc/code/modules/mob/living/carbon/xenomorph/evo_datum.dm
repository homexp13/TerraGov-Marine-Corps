...
/datum/strains_panel/ui_interact(mob/user, datum/tgui/ui)
	// Xeno only screen
	if(!isxeno(user))
		return
...
/datum/strains_panel/ui_state(mob/user)
	return GLOB.xeno_state

/// Static data provided once when the ui is opened
/datum/strains_panel/ui_static_data(mob/living/carbon/xenomorph/xeno)
	. = list()
	...
	.["evolves_to"] = list()
	for(var/evolves_into in xeno.xeno_caste.strains_to)
		var/datum/xeno_caste/caste = GLOB.xeno_strains_caste_datums[evolves_into][XENO_UPGRADE_BASETYPE]
...
/datum/strains_panel/ui_data(mob/living/carbon/xenomorph/xeno)
	. = list()
...
/datum/strains_panel/ui_act(action, list/params)
	. = ..()
	if(.)
		return
	...
	switch(action)
		if("evolve")
			var/datum/xeno_caste/caste = GLOB.xeno_strains_caste_datums[text2path(params["path"])][XENO_UPGRADE_BASETYPE]
